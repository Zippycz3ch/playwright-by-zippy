...
| `04_mira-ai-on-and-of-dashboard.spec.ts` | Toggles the agent OFF and back ON from the dashboard, handles popup dialogs, and verifies state         |
| `05_mira-ai-delete.spec.ts`              | Deletes the existing Mira AI agent and verifies no agents remain                                        |
| `06_mira-ai-E2E.spec.ts`                 | Full lifecycle E2E: create → behavior → handover → language → toggle on/off → delete                    |
...
| `@language`   | Covers welcome message language settings   |
| `@toggle`     | Covers toggling the agent on/off           |
| `@dashboard`  | Interaction from the AI Assistants list    |
...
npx playwright test tests/02_task01/04_mira-ai-on-and-of-dashboard.spec.ts --headed
npx playwright test tests/02_task01/05_mira-ai-delete.spec.ts --headed
